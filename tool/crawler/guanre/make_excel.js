...
for (let tableName in allTables) {
    ...
    for (let k in data) {
        ...
        if (keys.length === 0) {
            ...
            let dKey = Object.keys(data[k]);
            for (let i = 0; i < dKey.length; i++) {
                const dKeyElement = dKey[i];
                if(keys.indexOf(dKeyElement) === -1) {
                    keys.push(dKeyElement);
                }
            }
        }
    }
        ...
        for (let idx = 0; idx < keys.length; ++idx) {
            ...
            if (data[k][key]) {
                ...
                if (typeof data[k][key] === 'object') {
                    ...
                    line.push(JSON.stringify(data[k][key]).replace(new RegExp('\n', 'g'), '\\n'));
                    ...
                    if (data[k][key].indexOf('[') !== -1 && data[k][key].indexOf(']') !== -1) {
                        ...
                        line.push(data[k][key].replace(new RegExp(',', 'g'), '，').replace(new RegExp('\n', 'g'), '\\n'));
                    }
                    ...
                    if (typeof data[k][key] === 'string') {
                        line.push(data[k][key].replace(new RegExp('\n', 'g'), '\\n'));
                    }
                    else {
                        line.push(data[k][key]);
                    }
